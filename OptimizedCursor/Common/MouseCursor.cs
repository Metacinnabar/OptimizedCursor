    ...
    public static MouseCursor FromTexture2D(Texture2D texture, int originX, int originY)
    {
        ...
        try
        {
            ...
            if (surface == IntPtr.Zero)
                OptimizedCursor.LoggerInstance.Error("Failed to set surface (" + surface + ") for mouse cursor: " + SDL.SDL_GetError());
                //throw new InvalidOperationException("Failed to set surface (" + surface + ") for mouse cursor: " + SDL.SDL_GetError());

            handle = SDL.SDL_CreateColorCursor(surface, originX, originY);
            if (handle == IntPtr.Zero)
                OptimizedCursor.LoggerInstance.Error("Failed to set surface (" + surface + ") for mouse cursor (" + handle + "): " + SDL.SDL_GetError());
                //throw new InvalidOperationException("Failed to set surface (" + surface + ") for mouse cursor (" + handle + "): " + SDL.SDL_GetError());
        }
        finally
        {
            if (surface != IntPtr.Zero)
